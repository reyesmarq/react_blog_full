...
export default class ArticlesService {
  async getArticles(url = `${config.apiUrlL}/articles`) {
    const response = await Axios.get(url)

    return response.data.data
  }

  async getUserArticles(token, url = `${config.apiUrlL}/user/articles`) {
    const response = await Axios.get(url, {
      headers: {
  ...
  async deleteArticle(id, token) {
    await Axios.delete(`${config.apiUrlL}/articles/${id}`, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    })
  ...
  async getArticle(slug) {
    const response = await Axios.get(`${config.apiUrlL}/article/${slug}`)
  ...
  updateArticle = async (data, article, token) => {
    ...
    try {
      ...
      const response = Axios.put(`${config.apiUrlL}/articles/${article.id}`, {
        title: data.title,
        content: data.content,
        category_id: data.category,
